...
Changing \(\psi\) changes the prediction targets; changing \(\phi\) changes the noisy inputs and their weighting. Both therefore change the parameter update. Sections 2.1--2.3 learn the targets, dependencies, and schedules; Section 2.4 organizes transformer computations around the resulting conditional denoising tasks.
...
Sparse score dependencies and feature-space composition (Local Mechanisms \citep{local}). & Learn component divisions through task supervision and conditional denoising, then use their measured dependencies to organize both generation schedules and transformer information access. \\
...
\hypertarget{one-research-program-in-four-connected-steps}{%
\subsection{One research program in four connected steps}\label{one-research-program-in-four-connected-steps}}
...
\item
  \textbf{Learn denoising dependencies} by measuring which components help predict one another.
\item
  \textbf{Jointly optimize schedules and transformer activity} according to the information available at each noise configuration.
\item
  \textbf{Train shared and specialized computations} across conditional denoising tasks and diverse compositions.
\end{enumerate}
...
\caption{\textbf{Denoising dependencies guide schedules and module activity.} Make component j cleaner while holding target i's noise, the other inputs, and the transformer fixed. The change in i's flow-prediction error measures the benefit of information from j. Learned schedules balance these benefits against the difficulty of denoising a component before its useful context is available; module activity can depend on the same noise configuration. The curves and module assignments are illustrative.}
\label{fig:2}
\end{figure}

\hypertarget{train-shared-and-specialized-transformer-computations-for-compositional-denoising}{%
\subsection{2.4 Train shared and specialized transformer computations for compositional denoising}\label{train-shared-and-specialized-transformer-computations-for-compositional-denoising}}
...
\textbf{The core deliverable is a generation and revision method whose representation dependencies, denoising schedules, and transformer computations are understood together.} All four research steps use the same component interface and conditional prediction tasks.
