...
\bottomrule\noalign{}
\endlastfoot
Semantic feature generation and coordinated schedules (SFD \citep{sfd}, our LWD \citep{schedule} and trajectory optimization \citep{trajectory}). & Extend our joint schedule/denoiser learning to learned component divisions, with transformer module activity governed by the components' noise levels. \\
Sparse score dependencies and feature-space composition (Local Mechanisms \citep{local}). & Learn component divisions and refine their dependence structure together with the transformer and generation schedules. Component denoising losses connect the learned order to the information the model can use. \\
Modality-binding interventions, timestep specialization, and separated attention operations (Vision-Language Binding \citep{binding}, DeMe \citep{deme}, Compositional Attention \citep{compositional}). & Identify which computations can be shared across conditional denoising tasks and which must specialize as conditions, targets, and noise configurations change. Use those findings to support selective revision. \\
\end{longtable}
...
During a probe, we will jointly update a temporary denoiser and \(\phi\) with this objective. Both probe and main training will also include auxiliary flow losses at independently sampled component noise levels, including components held clean, to support dependency measurements and editing. Extractors remain fixed within a probe and are refined between probes. After refinement, we will fix the schedules for main training, following our LWD implementation \citep{schedule}; the main model uses the weighted flow-fitting term and auxiliary losses without the kinetic penalty. Our variational anisotropic method \citep{trajectory} supplies a complementary score-based formulation.
...
\section{3. Why the proposed work is feasible}\label{why-the-proposed-work-is-feasible}}
...
The representation-learning methods also have practical foundations. VAE/GAN \citep{vaegan} uses discriminator features for reconstruction, REPA \citep{repa} aligns diffusion features with pretrained representations, and ImageReward \citep{imagereward} supplies learned preference supervision. UniDiffuser \citep{unidiffuser} demonstrates training a shared transformer for different multimodal conditional generation tasks. Our proposed advance connects learned component divisions and schedules to shared or specialized transformer computations for those tasks.
...
Visual Genome \citep{visualgenome} & Objects, attributes, and relationships for training varied semantic compositions and checking preservation. \\
\end{longtable}
...
\textbf{Unpublished preliminary materials.} \emph{A Mechanistic View of Diffusion Transformers as Adaptive Patchwise Denoisers}, working manuscript; associated Experiment 1 and Experiment 3 reports; ELF-L experimental summary and accuracy records, 2026. These materials support the explicitly labeled preliminary results in Section 3.
