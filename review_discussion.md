...
**Latest approved revision: explicit early differentiation, a concrete sharing hypothesis, and H200 execution.** Following the third independent review and the user's approval, the proposal now places “Differentiation from the current state of the art” immediately after the contribution, with the research approach renumbered as Section 3. The distinction centers on measured conditional dependencies connecting representation availability, denoising progress, and shared or specialized transformer computation. The sharing study holds the image target fixed and adds a second visual conditioning component. The nested conditional-mean identity motivates a shared prediction core plus a conditional correction; it is a functional statement, not a theorem about particular weights. Equal specialization in other noise regimes and separate predictors distinguish the proposed sharing hypothesis from architectural capacity alone.

The controlled-variation comparison now includes directly tuned unequal-noise allocations with matched search and sampling allowances. Direct tuning can exploit dependencies; the comparison addresses the value of using explicit measurements to select allocations. Initial diffusion-transformer studies now use SiT-B scale (approximately 130M parameters). The PI supplied the availability of Duke's two H200 clusters. The revised workload is 5,600 H200 GPU-hours, with eight initial studies and a month-3 profiling gate. Cluster scheduling is not represented as a guaranteed reservation. The separate 10–20M task model on cached features is retained. No new preliminary experiments are required or run. The original USD 144,300 budget and workbook remain unchanged. Earlier compute estimates and open-access statements below are historical and superseded by this revision. The rebuilt PDF remains 11 pages, including the unchanged budget. Independent mathematical and visual checks found no blocking defects; clarified branch inputs and fixed regime assignments were incorporated. The equations and tables use the existing 11-point text, and the component definition and compute table stay together.

**Previous research revision: a concrete final model for generation and controlled variation.** Following the second independent review (`review_independent_round2.md`), the user clarified that language-state denoising principally supplies conditional tasks for learning dependencies. The draft now distinguishes those training targets from clean known prompt states used for image generation. The user also approved a creator-facing demonstration and asked that feasibility be grounded in Section 2.2. The final model generates a two-person scene and varies who holds an object while aiming to retain clothing, participant placement, and surrounding scene. Its initial variation procedure applies the existing forward corruption at component-specific noise levels, then uses the same conditional denoiser and rescaled schedules. All visual representation states and image latents may update. Preservation is measured, without assuming semantic slots, a valid cached-trajectory conditional sampler, or a new editor-training program.

The revision adds within-role nuisance controls, tests an unrestricted component–module prediction before imposing routes, includes small-model controls with permuted input access, defines role observability and visual-quality judgments, and distinguishes variation success conditional on a correct source from end-to-end success. The Latent Forcing discussion acknowledges its predictive-usefulness analysis, output experts, and strongest cascaded schedule. Existing-noise training motivates the controlled-variation interface; it does not establish that conflicting source information and a revised prompt will always yield the desired edit. A targeted second-agent check resolved remaining condition/target wording inconsistencies. No new preliminary experiments were run. The current PDF is 11 pages, including the unchanged budget, with the original font sizes. The named source of compute access remains an open factual item; none has been invented. Historical decisions below are retained.

**Latest budget decision: restore the original institution-routed budget.** The user explained that changing the budget would require another institutional approval and asked to retain the original figures. The proposal now requests USD 144,300 with 1.5 months of PI salary. `budget/Budget_Sony_Cheng.xlsx` is an exact copy of the original upload; the superseded revised workbook is removed from the active repository. The research plan retains the 5,000 H100-80GB GPU-hour estimate, run counts, and runtime assumptions, but removes cloud prices and any connection to a requested compute allocation. No replacement source of compute funding or confirmed access is asserted. Earlier budget-change discussions below remain historical and are superseded by this decision.
...
The coordinated revision incorporated fixed representation components, joint schedule/transformer learning, functional dependency diagnostics, explicit module access and sharing, a bounded causal case, the generated-state diagnostic, the hypothesis table, and a bounded execution plan. The initial design uses two fixed visual-coordinate groups, two fixed caption-coordinate groups, and the original image-latent component. Native PixArt image noising and VAE decoding are retained; the masked-caption diagnostic disables parallel visual paths and includes a frozen missing-word readout. An inference-only native-PixArt reference accompanies the four matched main variants.

The updated evidence uses 62.47% GSM8K accuracy with the approximately 800M denoiser and 121M encoder. The proposal includes two compact figures and 11-point body text. The initial coordinated revision's USD 149,999 budget is superseded by the restoration noted above. No additional preliminary experiment was run. A fresh reviewer read the revised proposal without this discussion and helped resolve the remaining masked-caption, baseline, and figure-metric details. The PDF has nine narrative/reference pages and one budget page. The PI CV remains a separate submission item; the original workbook dates are retained as requested.
