...
![Figure 2: example dependency DAG, asynchronous schedules, and transformer module activity](figures/fig2_recovery_schedule.png)

**Figure 2. An illustrative dependence structure connects representation, schedule, and module activity.** (A) Arrows show information used to predict downstream components, conditioned on the prompt's giver–receiver assignment. This is one possible decomposition and factorization. (B) Corresponding component groups follow overlapping schedules: progress 0 is noise and 1 is a clean state. (C) The same transformer uses different attention heads and MLP slices at the marked times; colored modules are active and gray modules are gated off. Shared backbone computations are omitted. The project learns the components, dependencies, schedules, and module roles illustrated here.
...
**Localize computations as well as generation stages.** In Figure 2's illustrative ordering, participant and cup information becomes available early, clothing bindings develop using participant information, and handover predictions combine the participants, their attributes, and the cup with the giver–receiver roles supplied by the prompt. A head that binds clothing to a participant may be useful at different noise levels from one that combines participants and object information into the handover. Shared computations can remain active throughout. Learned schedules can overlap and cross; module activity depends on the full progress vector $\mathbf u$, rather than a fixed global time interval.
